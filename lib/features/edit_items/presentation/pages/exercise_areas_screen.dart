...
class _ExerciseAreasScreenState extends State<ExerciseAreasScreen> {
  ...
  Widget _buildBody(BuildContext context, EditItemsViewModel viewModel) {
    ...
    return CustomScrollView(
      slivers: [
        ...
        // iCloud Sync Buttons
        SliverToBoxAdapter(
          child: _buildSyncButtons(context, viewModel),
        ),
  ...
  Widget _buildSyncButtons(BuildContext context, EditItemsViewModel viewModel) {
    return Padding(
                ...
                children: [
                  Expanded(
                    child: CupertinoButton(
                      ...
                      onPressed: () => _downloadFromICloud(context, viewModel),
                      child: const Row(
                  ...
                  Expanded(
                    child: CupertinoButton(
                      ...
                      borderRadius: BorderRadius.circular(12),
                      padding: const EdgeInsets.symmetric(vertical: 12, horizontal: 16),
                      onPressed: () => _uploadToICloud(context, viewModel),
                      child: const Row(
                        mainAxisAlignment: MainAxisAlignment.center,
                        children: [
                          Icon(
  ...
  Future<void> _downloadFromICloud(BuildContext context, EditItemsViewModel viewModel) async {
    try {
      ...
      final icloudService = LocalStorageService.icloudSyncService;
      if (icloudService == null) {
        if (context.mounted) Navigator.of(context).pop(); // Close loading dialog
        if (context.mounted) _showErrorDialog(context, 'iCloud sync is not available');
        return;
      }
      ...
      for (final fileName in filesToDownload) {
        await icloudService.downloadFile(fileName);
      }

      if (context.mounted) Navigator.of(context).pop(); // Close loading dialog
      
      // Refresh the data
      await viewModel.fetchPracticeAreas();
      
      if (context.mounted) _showSuccessDialog(context, 'Successfully downloaded latest data from iCloud');
    } catch (e) {
      if (context.mounted) Navigator.of(context).pop(); // Close loading dialog
      if (context.mounted) _showErrorDialog(context, 'Failed to download from iCloud: $e');
    }
  }

  Future<void> _uploadToICloud(BuildContext context, EditItemsViewModel viewModel) async {
    try {
      // Show loading indicator
      showCupertinoDialog(
        context: context,
      ...
      if (result.success) {
        if (context.mounted) _showSuccessDialog(context, 'Successfully uploaded all data to iCloud');
      } else {
        if (context.mounted) _showErrorDialog(context, result.error ?? 'Unknown error occurred');
      }
    } catch (e) {
      if (context.mounted) Navigator.of(context).pop(); // Close loading dialog
      if (context.mounted) _showErrorDialog(context, 'Failed to upload to iCloud: $e');
    }
  }
