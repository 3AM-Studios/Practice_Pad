...
class ChordSymbol {
  ...
  static String formatChordQuality(String quality) {
    ...
    return quality
        ...
        .replaceAll(
            '7', '⁷'); // Dominant 7th (keep this last to avoid conflicts)
  }

  /// Determines if this chord is diatonic to the current key signature
  bool get isDiatonic {
    if (originalKeySignature == null)
      return true; // If no key context, assume diatonic
    ...
    final expectedQualities =
        _getDiatonicChordQualities(scaleDegree, isMinorKey);
  ...
  bool _isKeyMinorForSignature(KeySignatureType keySignature) {
    return keySignature.name.contains('Minor') ||
        keySignature.name.contains('minor');
  }
  ...
  Set<String> _getDiatonicChordQualities(int scaleDegree, bool isMinorKey) {
    if (isMinorKey) {
      ...
      switch (scaleDegree) {
        case 1:
          return {'min', 'min7', 'm9'}; // i, imin7, im9
        case 2:
          return {'dim', 'm7b5'}; // ii°, iiø7
        case 3:
          return {'maj', 'maj7', 'maj9'}; // III, IIImaj7, IIImaj9
        case 4:
          return {'min', 'min7', 'm9'}; // iv, ivmin7, ivm9
        case 5:
          return {'min', 'min7', 'm9'}; // v, vmin7, vm9 (natural minor)
        case 6:
          return {'maj', 'maj7', 'maj9'}; // VI, VImaj7, VImaj9
        case 7:
          return {'maj', '7', '9'}; // VII, VII7, VII9 (dominant)
        default:
          return {};
      }
    } else {
      ...
      switch (scaleDegree) {
        case 1:
          return {'maj', 'maj7', 'maj9'}; // I, Imaj7, Imaj9
        case 2:
          return {'min', 'min7', 'm9'}; // ii, iimin7, iim9
        case 3:
          return {'min', 'min7', 'm9'}; // iii, iiimin7, iiim9
        case 4:
          return {'maj', 'maj7', 'maj9'}; // IV, IVmaj7, IVmaj9
        case 5:
          return {'maj', '7', '9'}; // V, V7, V9 (dominant)
        case 6:
          return {'min', 'min7', 'm9'}; // vi, vimin7, vim9
        case 7:
          return {'dim', 'm7b5'}; // vii°, viiø7
        default:
          return {};
      }
    }
  ...
  bool _isKeyMinor() {
    ...
    return originalKeySignature!.name.contains('Minor') ||
        originalKeySignature!.name.contains('minor');
  }
  ...
  List<Pitch> generateChordNotes() {
    ...
    final intervals =
        qualityIntervals[effectiveQuality] ?? qualityIntervals['maj']!;
  ...
  String _intervalToRomanNumeral(int interval, bool isMinorKey) {
    ...
    switch (interval) {
      ...
      case 6:
        baseNumeral = '♯IV';
        break; // Sharp fourth / Flat fifth (tritone)
      case 7:
        baseNumeral = 'V';
        break; // Fifth
      case 8:
  ...
  String _adjustRomanNumeralForQuality(
      String baseNumeral, int interval, bool isMinorKey) {
    // Remove any existing quality symbols from the base numeral
    String cleanNumeral =
        baseNumeral.replaceAll('°', '').replaceAll('ø', '').replaceAll('+', '');
    ...
    switch (effectiveQualityValue) {
      ...
      default:
        ...
        if (effectiveQualityValue.contains('m') &&
            !effectiveQualityValue.contains('maj')) {
          result = cleanNumeral.toLowerCase();
        } else if (effectiveQualityValue.contains('dim')) {
          result = cleanNumeral.toLowerCase();
        } else if (effectiveQualityValue.contains('aug') ||
            effectiveQualityValue.contains('+')) {
          result = cleanNumeral.toUpperCase();
        } else {
          result = cleanNumeral.toUpperCase();
        }
  ...
  Widget buildWidget({
    ...
    final isCurrentChord = index != null &&
