...
class GlyphMetadata {
  ...
  Offset stemRootOffset(String noteHeadMetadataKey, {required bool isStemUp}) {
    ...
    // Apply the same scaling factor as glyph paths: (staffSpace/250) * 0.7
    const double originalStaffSpace = 250.0;
    final double scaleFactor = (Constants.staffSpace / originalStaffSpace) * 0.7;
    return Offset(stemRoot[0] as double, -stemRoot[1] as double) * 
        (originalStaffSpace * scaleFactor);
  }
