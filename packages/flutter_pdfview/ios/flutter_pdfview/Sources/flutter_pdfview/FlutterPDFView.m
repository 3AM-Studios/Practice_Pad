...
- (void)onMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    } else if ([[call method] isEqualToString:@"getViewState"]) {
        [_pdfView getViewState:call result:result];
    } else {
...
- (instancetype)initWithFrame:(CGRect)frame
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(handleViewStateChanged:) name:PDFViewScaleChangedNotification object:_pdfView];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(handleViewStateChanged:) name:PDFViewDisplayModeChangedNotification object:_pdfView];
...
- (void)getViewState:(FlutterMethodCall*)call result:(FlutterResult)result {
    PDFPage* currentPage = _pdfView.currentPage;
    if (currentPage != nil) {
        CGRect pageRect = [currentPage boundsForBox:kPDFDisplayBoxMediaBox];
        NSDictionary* viewState = @{
            @"zoom": @(_pdfView.scaleFactor),
            @"offsetX": @(0.0), // iOS PDFView doesn't expose scroll offset directly
            @"offsetY": @(0.0),
...
-(void)handleViewStateChanged:(NSNotification*)notification {
    PDFPage* currentPage = _pdfView.currentPage;
    if (currentPage != nil) {
        CGRect pageRect = [currentPage boundsForBox:kPDFDisplayBoxMediaBox];
        NSDictionary* viewState = @{
        ...
        [_controller invokeChannelMethod:@"onViewStateChanged" arguments:viewState];
    }
}
