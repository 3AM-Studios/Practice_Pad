...
public class FlutterPDFView implements PlatformView, MethodCallHandler {
            ...
            config
                    ...
                    .onDraw(new OnDrawListener() {
                        ...
                        public void onLayerDrawn(Canvas canvas, float pageWidth, float pageHeight, int displayedPage) {
                            // Notify Flutter about view state changes
                            Map<String, Object> viewState = new HashMap<>();
                            viewState.put("zoom", pdfView.getZoom());
                            viewState.put("offsetX", pdfView.getCurrentXOffset());
                            viewState.put("offsetY", pdfView.getCurrentYOffset());
                            viewState.put("pageWidth", pageWidth);
                            viewState.put("pageHeight", pageHeight);
                            viewState.put("displayedPage", displayedPage);
                            methodChannel.invokeMethod("onViewStateChanged", viewState);
                        }
                    })
                    .load();
        }
    ...
    void getViewState(Result result) {
        Map<String, Object> viewState = new HashMap<>();
        viewState.put("zoom", pdfView.getZoom());
        viewState.put("offsetX", pdfView.getCurrentXOffset());
        viewState.put("offsetY", pdfView.getCurrentYOffset());
        viewState.put("pageWidth", pdfView.getOptimalPageWidth());
        viewState.put("pageHeight", pdfView.getOptimalPageHeight());
        result.success(viewState);
    }
